#  NMIS - ConnectWise Connector v.1.4.0 - 12 June 2020 Oscar Berlanga
...
package Notify::connectwise_connector;
...
if ($@) {
    logMsg("ERROR Connectwise authentication method requires Mojo::UserAgent but module not available: $@!");
    return 0;
}
...
sub sendNotification {
	...
	if ($cw_server eq "" || $company_id eq "" || $public_key eq "" || $private_key eq "" || $cw_clientId eq "" || $ticket_companyId eq "") {
	    logMsg("ERROR one or more required ConnectWise variables are missing from Config.nmis");
	    exit 0;
	}
	...
	if($event->{stateless} ne "false"){
		logMsg("Not a stateful event.");
		return 1;
	}
	...
	if($message =~ /closed|normal/i){
		if($isNew){
			logMsg("No previous records, I can't close an event that I haven't created");
		}else{
			if($data->{$node}->{events}){
				...
				if(not defined $ticketID){
					logMsg("No MATCH found for this event");
					...
					if($result){
						...
						writeToFile($toSave);
						logMsg("Event removed from File");
					}
				}
			}else{
				logMsg("NO Previous records, I can't close an event that I haven't created");
		...
		if ($data->{$node}->{events}) {
			...
			if($exist){
				logMsg("Event $eventName already exist, I can't add it again");
				return;
			}
	...
	sub create_ticket{
		...
		my $summary ="Node: $node at $NI->{host} Event: $event->{event} $event->{element}";

		my $description = "Node: $node at $NI->{host}
...
NMIS Sever: $config->{nmis_host}";
		...
		$ticket->{location}->{name} = $NI->{location};
	    ...
	    if ($res->is_success) {
	    	logMsg("INFO ConnectWise, Ticket $TID->{id} successfully created!");
	    	return $TID->{id};
	    }
	    elsif ($res->is_error) {
	    	logMsg("ERROR ConnectWise responded with: ". $res->message);	
	    	return;
	    }
	    else {
		    logMsg("ERROR ConnectWise response failed");
		    return 
	    }
	}
	...
	sub close_ticket{
		...
		if ($res->is_success) {
			logMsg("INFO ConnectWise, Got Ticket details\n");
			...
			if ($update_res->is_success) {
				logMsg("INFO ConnectWise, Ticket ". $ticketID ." successfully closed.\n");
				return 1;
			}
