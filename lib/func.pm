...
# cache table
my $C_cache = undef; # that's the configuration structure cache
my $C_modtime = undef;
my %Table_cache;
my $Config_cache = undef;				# that's the configuration name, e.g. Config.
my $confdebug = 0;
my $nmis_var;
my $nmis_conf;
my $nmis_models;
...
#
# attention: loadtable does NOT support create-on-the-fly with lock, even though readfiletohash does!
sub loadTable {
	my %args = @_;
...
# args: file, lock, json
# returns: nothing, or hashref or (hashref,handle)
# note: if lock is true, a missing file is created on the fly!
#
# fixme: passing json=false DOES NOT WORK if the config says use_json=true
sub readFiletoHash {
	my %args = @_;
...
sub selftest
{
	...
	if (getbool($config->{daemon_fping_active}) && !$fpingd_found)
	{
...
{
	my (%args) = @_;
	return "no config loaded, cannot determine log directory!" if (!$C_cache);
  ...
  my @output = (  returnDateStamp($args{when}||time),
	...
	my $res = setFileProtDiag(file => $auditlogfile);
